...
// Added unit_price to the sort field options
...
export default function AdminTransactionsPage() {
  ...
  const [dialogAction, setDialogAction] = useState<'update' | 'void'>('void');
  ...
  const calculateAmountPaid = (transaction: Transaction) => {
    ...
    // Return the amount directly without additional formatting
    return amount;
  };

  const handleActionClick = (transaction: Transaction, action: 'update' | 'void') => {
  ...
  const handleProcessAction = async () => {
    ...
    try {
      ...
      if (dialogAction === 'void') {
        await dashboardApi.voidTransaction(selectedTransaction._id);
      ...
      toast.error(`Failed to process transaction action`);
    } finally {
  ...
  const handleUpdateTransaction = async () => {
    ...
    try {
      ...
      const updateData = {
        ...
        paid_amount: newTotalPrice // Update paid amount to match the new total
      };
  ...
  const getSummaryMetrics = () => {
    if (!transactions.length) return { total: 0, completed: 0, pending: 0, totalAmount: 0 };
  ...
  return (
    <div className="p-4 md:p-6 space-y-6">
      ...
      >
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-slate-900">Transaction History</h1>
          <p className="text-slate-500 mt-1">Manage and track all copra transactions</p>
        </div>
                  ...
                  <DropdownMenuContent align="end" className="w-40">
                    {statusOptions.map(option => (
                      <DropdownMenuItem
                        ...
                        className={statusFilter === option.value ? "bg-amber-50" : ""}
                      >
                        {option.label}
                      </DropdownMenuItem>
                    ))}
                              ...
                              {transaction.status === 'completed' && (
                                <div className="flex space-x-2">
                                  <Button
                                    ...
                                    onClick={() => handleActionClick(transaction, 'update')}
                                  >
                                  ...
                                  >
                                    <Trash className="h-3.5 w-3.5 mr-1" /> Void
                                  </Button>
                                </div>
                              )}
                            </td>
                ...
                {transaction.status === 'completed' && (
                  <div className="p-4 pt-0 flex space-x-2">
                    <Button
                      variant="outline"
                      size="sm"
                      className="flex-1 h-9 text-blue-600 border-blue-200 hover:bg-blue-50 hover:text-blue-700"
                      onClick={() => handleActionClick(transaction, 'update')}
                    >
            ...
            <AlertDialogDescription>
              ...
              <div className="mt-4 p-4 bg-slate-50 rounded-md space-y-2 text-sm">
                ...
                <div className="grid grid-cols-2">
                  ...
                  <span>{selectedTransaction ? getSupplierName(selectedTransaction.supplier) : 'Unknown'}</span>
                </div>
                {/* Added Unit Price */}
                <div className="grid grid-cols-2">
                  <span className="font-medium">Unit Price:</span>
                  <span>{selectedTransaction && formatCurrency(selectedTransaction.unit_price)}</span>
                </div>
                <div className="grid grid-cols-2">
                  <span className="font-medium">Amount:</span>
                  <span>
                    {selectedTransaction && formatCurrency(selectedTransaction.total_amount || selectedTransaction.total_price)}
                  </span>
                </div>
                {/* Loan deduction info */}
                  ...
                  <>
                    <div className="grid grid-cols-2">
                      ...
                      <span className="text-amber-600">
                        {formatCurrency(selectedTransaction.loan_deduction)}
                      </span>
                    </div>
                    <div className="grid grid-cols-2">
